	...
	return (
		<div className="container page-stack">
			<section className="hero-panel compact">
				...
				<p className="hero-copy">View all orders created using your logged in account.</p>
			</section>
			...
			{!loading && !error && !orders.length && <p className="status">No orders found.</p>}

			<section className="cart-list">
				{orders.map((order) => (
					<article className="cart-item" key={order.id || `${order.userId}-${order.total}`}>
						<p><strong>Order ID:</strong> {order.id ?? "N/A"}</p>
						<p><strong>User ID:</strong> {order.userId ?? userId}</p>
						<p>
							<strong>Total:</strong> ₹{new Intl.NumberFormat("en-IN").format(resolveOrderTotal(order))}
						</p>
